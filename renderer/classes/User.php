...
class User
{
    ...
    {
        $this->blender->importBlend(config('AVATARS', ($this->user->tool) ? 'TOOL' : 'DEFAULT'));
	$this->setAngle();
        $this->generateAvatar();
        $this->blender->importModel('tool', $this->user->tool);
	...
	if ($this->user->head) {
	    ...
	    $this->blender->importModel('Head', $this->user->head);
            $this->blender->setTexture('face', 'Head', $this->user->face ?? 'default');
	}
