...
class ForumController extends Controller
{
    public function index()
    {
        ...
        $excludedTopicIds = [15, 6]; // dont show conspiracies and the admin in new threads

        $recentThreads = ForumThread::where('is_deleted', '=', false)
        ->whereNotIn('topic_id', $excludedTopicIds)
...
{
    $popularThreads = DB::table('forum_replies')
        ->select('thread_id', DB::raw('COUNT(*) as replies_count'))
    ...
    return view('web.forum.popular')->with('popularThreads', $popularThreads);
}
