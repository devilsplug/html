...
class SettingsController extends Controller
{
    ...
    {
        $user = Auth::user();
        $email = preg_replace('/[^@]+@([^\s]+)/', substr($user->email, 0, 3) . '********@1', $user->email);
        $themes = config('themes.list');

        $membershipMessage = $this->getMembershipMessage($user->membership_until);
        ...
        switch ($request->type) {
            case 'username':
                ...
                $this->validate($request, [
                    'username' => ['min:2', 'max:26', 'regex:/\\A[a-z\\d]+(?:[.-][a-z\\d]+)*\\z/i', 'unique:users'],
                ], [
    ...
    private function getMembershipMessage($membershipUntil)
    {
        if ($membershipUntil) {
            $formattedDate = Carbon::parse($membershipUntil)->format('M d Y');
            return "Your membership will expire on $formattedDate";
        } else {
            return "You currently don't have a membership!";
        }
    }
}