            ...
            <div class="card">
                ...
                <div class="content">
                    ...
                    <div class="block">
                        ...
                        <i class="f-right light-gray-text far fa-edit" style="cursor:pointer;"
                            data-modal-open="username"></i>
                    </div>
                    <div class="block">
                        <span class="dark-gray-text" style="padding-right:5px;">Password:</span>
                        <span class="light-gray-text">*********</span>
                        <i class="f-right light-gray-text far fa-edit" style="cursor:pointer;"
                            data-modal-open="password"></i>
                    </div>
                    <div class="block">
                        <span class="dark-gray-text" style="padding-right:5px;">Email:</span>
                        <span
                            class="{{ Auth::user()->hasVerifiedEmail() ? 'light-gray' : 'red' }}-text">{{ $email ?? 'None' }}</span>
                            ...
                            <div class="inline">
                                <select name="theme">
                                    @foreach ($themes as $key => $theme)
                                        <option value="{{ $key }}"
                                            {{ Auth::user()->setting->theme == $key ? 'selected' : '' }}
                                            {{ !$theme['available'] ? 'disabled' : '' }}>{{ $theme['name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <hr>
                    <div>
                        <span class="dark-gray-text bold block" style="padding-bottom: 5px;"> Billing </span>
                        <div>
                            <div><span class="dark-gray-text">{{ $membershipMessage }}</span></div>
                        </div>
                        <div style="padding-bottom: 5px;">
                            <span class="dark-gray-text">Stripe Settings:</span>
                            <button class="f-right small blue button" disabled> Billing Portal </button>
                        </div>
                    </div>
                    <hr>
                    ...
                    <form action="{{ route('account.settings.update') }}" method="POST">
                        ...
                        <textarea name="description" placeholder="Hi, my name is {{ Auth::user()->username }}" class="width-100 block"
                            style="height:80px;margin-bottom:6px;">{{ Auth::user()->description }}</textarea>
                ...
                <div class="modal" style="display:none;" data-modal="username">
                    <div class="modal-content">
                        <form action="{{ route('account.settings.update') }}" style="display:inline;" method="POST">
                            ...
                            <span style="color:red;font-size:11px;">WARNING: This will take
                                {{ number_format(config('site.username_change_price')) }} bucks</span>
